...
import factoryClass.BattleGameAbstractFactory;
import factoryClass.LandGameConcreteFactory;
import factoryClass.SeaGameConcreteFactory;
		...
		while(numBattleItems<MAX_BATTLE_ITEMS) {
			for(int i=0;i<MAX_BATTLE_ITEMS;i++) {
				...
				if(isFreeCell(randomNum) && numBattleItems<MAX_BATTLE_ITEMS) {
					vehicleLocation[randomNum]=gameFactory.getVehicle();
